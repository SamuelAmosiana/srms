...
$stmt = $pdo->query("SELECT COALESCE(SUM(amount),0) as total FROM finance_transactions WHERE type = 'income'");
$stats['total_fees_collected'] = $stmt->fetch()['total'] ?? 0;

// Total outstanding fees (assuming a fees table; adjust query as needed)
$stmt = $pdo->query("SELECT COALESCE(SUM(pf.fee_amount),0) as total_due
...
$stmt = $pdo->query("SELECT COALESCE(SUM(amount),0) as total FROM finance_transactions WHERE type = 'expense'");

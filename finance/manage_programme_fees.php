...
<head>
    ...
    <style>
        ...
        .breakdown-content .programme-section {
            margin: 10px;
            border: 1px solid var(--border-color);
            border-radius: 8px;
        }
        
        .breakdown-content .fee-table {
            margin: 0;
        }
        
        .breakdown-content .programme-header {
            background: #f8f9fa;
            color: #333;
        }
                ...
                <table class="fee-table" id="main-programme-table">
                    ...
                    <tbody>
                        <?php foreach ($grouped_fees as $programme_id => $group): ?>
                            <?php 
                                $total_per_term = 0.0;
                                foreach ($group['fees'] as $f) {
                                    if ((!isset($f['is_active']) || $f['is_active'] == 1) && $f['fee_type'] === 'per_term') {
                                        $total_per_term += (float)$f['fee_amount'];
                                    }
                                }
                                $duration_text = 'N/A';
                                try {
                                    $dStmt = $pdo->prepare("SELECT duration, duration_unit FROM programme WHERE id = ?");
                                    $dStmt->execute([$programme_id]);
                                    $pinfo = $dStmt->fetch();
                                    if (!empty($pinfo['duration']) && !empty($pinfo['duration_unit'])) {
                                        $duration_text = $pinfo['duration'] . ' ' . $pinfo['duration_unit'];
                                    }
                                } catch (Exception $e) {}
                            ?>
                            <tr>
                                <td><?php echo htmlspecialchars($group['programme_name']); ?></td>
                                <td><?php echo number_format($total_per_term, 2); ?></td>
                                <td><?php echo htmlspecialchars($duration_text); ?></td>
                                <td>
                                    <button class="btn small" onclick="toggleBreakdown(<?php echo $programme_id; ?>)" title="View fee breakdown"><i class="fas fa-eye"></i> View</button>
                                </td>
                            </tr>
                            <!-- Hidden row to show fee breakdown -->
                            <tr id="breakdown-<?php echo $programme_id; ?>" class="breakdown-row" style="display: none;">
                                <td colspan="4" class="breakdown-content">
                                    <div class="programme-section">
                                        <div class="programme-header">
                                            <?php echo htmlspecialchars($group['programme_name']); ?> - Fee Breakdown
                                        </div>

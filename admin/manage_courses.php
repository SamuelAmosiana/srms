                    ...
                    if (!in_array($file['type'], $allowed_types) && pathinfo($file['name'], PATHINFO_EXTENSION) !== 'csv') {
                        ...
                        if (($handle = fopen($file['tmp_name'], 'r')) !== FALSE) {
                            ...
                            while (($data = fgetcsv($handle, 1000, ',')) !== FALSE) {
                                ...
                                if (count($data) >= 4) {
                                    $code = trim(strtoupper($data[0]));
                                    $name = trim($data[1]);
                                    $programme_name = trim($data[2]);
                                    $credits = trim($data[3]);
                                    $description = isset($data[4]) ? trim($data[4]) : '';
                                    
                                    // Validate data
                                    if (empty($code) || empty($name) || empty($programme_name) || empty($credits)) {
                                        $errors[] = "Line {$line_number}: Missing required fields";
                                        continue;
                                    }
                                    
                                    if (strlen($code) > 20) {
                                        $errors[] = "Line {$line_number}: Course code too long (max 20 characters)";
                                        continue;
                                    }
                                    
                                    if (!is_numeric($credits) || $credits <= 0) {
                                        $errors[] = "Line {$line_number}: Credits must be a positive number";
                                        continue;
                                    }
                                    
                                    // Find programme ID
                                    $programme_stmt = $pdo->prepare("SELECT id FROM programme WHERE name = ?");
                                    $programme_stmt->execute([$programme_name]);
                                    $programme = $programme_stmt->fetch();
                                    
                                    if (!$programme) {
                                        $errors[] = "Line {$line_number}: Programme '{$programme_name}' not found";
                                        continue;
                                    }
                                    ...
                                    if ($check_stmt->rowCount() > 0) {
                                        $errors[] = "Line {$line_number}: Course code '{$code}' already exists";
                                        continue;
                                    }
                                    
                                    // Insert course
                                    try {
                                        $insert_stmt = $pdo->prepare("INSERT INTO course (name, code, programme_id, credits, description, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
                                        $insert_stmt->execute([$name, $code, $programme['id'], $credits, $description]);
                                        $success_count++;
                                        $uploaded_courses[] = $code;
                                    } catch (Exception $e) {
                                        $errors[] = "Line {$line_number}: Database error - " . $e->getMessage();
                                    }
                                } else {
                                    $errors[] = "Line {$line_number}: Invalid number of columns (expected at least 4)";
                                }
                            }
                            fclose($handle);
                            ...
                            if ($success_count > 0) {
                                $message = "Successfully uploaded {$success_count} courses";
                                if (!empty($uploaded_courses)) {
                                    $message .= ": " . implode(', ', array_slice($uploaded_courses, 0, 5));
                                    if (count($uploaded_courses) > 5) {
                                        $message .= " and " . (count($uploaded_courses) - 5) . " more";
                                    }
                                }
                                $messageType = 'success';
                            }
                            ...
                            if (!empty($errors)) {
                                if ($success_count > 0) {
                                    $message .= ". However, there were " . count($errors) . " errors.";
                                } else {
                                    $message = "Upload failed with " . count($errors) . " errors. ";
    ...
    <main class="main-content">
        ...
        <div class="action-bar">
            ...
            <div class="action-right">
                <?php if (!isset($_GET['view'])): ?>
                    <button onclick="showBulkUploadModal()" class="btn btn-info">
        ...
        <div id="bulkUploadModal" class="modal">
            <div class="modal-content">
                ...
                <h2><i class="fas fa-file-upload"></i> Bulk Upload Courses</h2>
                <p>Upload multiple courses at once using a CSV file.</p>
                
                <div class="upload-template">
                    <div class="template-header">CSV Template Format:</div>
                ...
                <form method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="action" value="bulk_upload_courses">
                    <div class="form-group">
                        <label for="csv_file">Select CSV File *</label>
                        <input type="file" id="csv_file" name="csv_file" accept=".csv" required>
                        <small class="form-text">Maximum file size: 2MB. Only CSV files are accepted.</small>
                    </div>
                    <div class="form-actions">
                        <button type="submit" class="btn btn-green">
                            <i class="fas fa-upload"></i> Upload Courses
                        </button>
                        <button type="button" onclick="closeBulkUploadModal()" class="btn btn-orange">
